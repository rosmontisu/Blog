...
# 3. 정리
- 언리얼 오브젝트 <-> 클래스 정보를 담은 `UClass` 객체가 항상 매칭되어 있다.
	- `UCLSS()` 매크로는 `UHT`가 언리얼 오브젝트를 리플렉션 시스템에 등록하도록 표시해준다.
	- 이 과정으로 모든 `UObject`기반 클래스는 자신을 설명하는 UClass 객체를 하나 갖게 된다.
-  `UClass`로부터 언리얼 오브젝트의 정보를 파악할 수 있다.
- `UClass` -> `클래스 메타데이터` + `CDO`
	- 메타데이터: 클래스 자체의 정보(클래스이름, 함수목록, 프로퍼티 목록 등)
	- CDO: 기본값을 담은 템플릿 객체
		- `UClass`는 CDO의 포인터를 가지고 있음
- 클래스 정보와 CDO는 엔진 초기화 과정에서 생성되므로 게임 개발에서 안전하게 사용 가능
- 헤더 정보를 변경 or 생성자 정보를 변경시 데이터를 끄고 컴파일하는게 안전
