---
title: '[이득우의 UE5 C++] 5. UObject Reflection 시스템'
...
# 3. 정리
- 언리얼 오브젝트는 항상 자신을 설명하는 `UClass`객체와 매칭되어 있다.
	- `UCLSS()` 매크로는 UHT가 해당 클래스를 리플렉션 시스템에 등록하도록 표시해준다.
	- 이 과정으로 모든 `UObject`기반 클래스는 자신을 설명하는 UClass 객체를 하나 갖게 된다.
-  `UClass`는 클래스 메타데이터와 CDO를 함께 관리한다.
- `UClass` -> `클래스 메타데이터` + `CDO`
	- 메타데이터: 클래스 자체의 정보(클래스이름, 함수목록, 프로퍼티 목록 등)
	- CDO(Class Default Object): 기본값을 담은 템플릿 객체, 인스턴스 생성 시 초기화 기준이 됨
		- `UClass`는 CDO의 포인터를 가지고 있어, 런타임에 접근이 가능한 것
- 클래스 정보와 CDO는 엔진 초기화 과정에서 자동으로 생성되므로 안전하게 사용 가능
- 헤더 정보를 변경 or 생성자에서 CDO 기본값 수정시 에디터를 종료 후 다시 컴파일해야 정상 반영
